...
class CallRepository {
  ...
  Future<void> createCallDocument({
    ...
    debugPrint('📝 [CallRepository] Creating call document: appointmentId=$appointmentId, doctorId=$doctorId, userId=$userId');
  ...
  Stream<String?> watchCallStatus({required String callId}) =>
      _firestore.collection('calls').doc(callId).snapshots().map(
    ...
    required OnError onError,
      ...
      RtcEngineEventHandler(
        ...
        onJoinChannelSuccess: (RtcConnection connection, int elapsed) =>
            debugPrint('✅ [Agora] Joined channel: ${connection.channelId} after ${elapsed}ms'),
        onUserJoined: (RtcConnection connection, int remoteUid, int elapsed) {
          debugPrint('👤 [Agora] Remote user joined: $remoteUid');
          onRemoteUserJoined(remoteUid);
        },
        onUserOffline: (RtcConnection connection, int remoteUid, UserOfflineReasonType reason) {
          debugPrint('👋 [Agora] Remote user left: $remoteUid (${reason.name})');
        ...
        onLocalVideoStateChanged: (VideoSourceType source, LocalVideoStreamState state, LocalVideoStreamReason reason) =>
            debugPrint('📹 [Agora] Local video state: ${state.name} reason: ${reason.name}'),
        onRemoteVideoStateChanged: (RtcConnection connection, int remoteUid, RemoteVideoState state, RemoteVideoStateReason reason, int elapsed) =>
            debugPrint('📺 [Agora] Remote video state: ${state.name} reason: ${reason.name}'),
        onFirstRemoteVideoFrame: (RtcConnection connection, int remoteUid, int width, int height, int elapsed) =>
            debugPrint('🖼️ [Agora] First remote frame: uid=$remoteUid ${width}x$height'),
      );
  ...
  Future<void> initAndJoin({
    ...
    try {
      debugPrint('🎬 [CallRepository] Initializing RTC engine');
      ...
      debugPrint('🎥 [CallRepository] Camera preview started');

      if (!_isJoined) {
        _isJoined = true;
        final localUid = _uidFromString(userId);
        debugPrint('🚀 [CallRepository] Joining channel: $channelName with uid=$localUid');
  ...
  Future<void> leaveAndDispose() async {
    ...
    try {
      ...
      debugPrint('👋 [CallRepository] Channel left');
      await _engine?.stopPreview();
      debugPrint('🎥 [CallRepository] Preview stopped');
      if (_eventHandler != null) {
        _engine?.unregisterEventHandler(_eventHandler!);
        _eventHandler = null;
      }
      await _engine?.release();
      debugPrint('🗑️ [CallRepository] Engine released');
    } catch (e) {
  ...
  Widget buildRemoteView({required String channelName, required int remoteUid}) {
    if (_engine == null) {
      debugPrint('⚠️ [CallRepository] buildRemoteView called with null engine');
      return const SizedBox.shrink();
    }
    ...
    debugPrint('🎥 [CallRepository] Creating remote view: uid=$remoteUid channel=$channelName');
    _cachedRemoteUid = remoteUid;
    _cachedRemoteChannel = channelName;
